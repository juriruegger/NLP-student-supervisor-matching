...
while len(research_outputs) < count:
    ...
    print(f"research outputs fetched: {len(research_outputs)}")
print(f"Total research outputs fetched: {len(research_outputs)}")
...
for researcher in researchers:
    ...
    for abstract in researcher.get("abstracts", []):
        ...
        if not abstract.get("abstract").get("en_GB"):
            continue
        if abstract.get("abstract").get("en_GB") == "":
            continue
        print("title:", abstract.get("title"))
        processed_abstracts += 1
        print("Abstracts processed:", processed_abstracts)
...
all_keywords = set()
print("keywords for persons")
for person in persons:
    if not isinstance(person, dict):
        continue
    for keyWordGroup in person.get("keywordGroups", []):
        if not isinstance(keyWordGroup, dict):
            continue
        for keywordObj in keyWordGroup.get("keywords", []):
            if not isinstance(keywordObj, dict):
...
print("keywords for research outputs")

for output in research_outputs:
    if not isinstance(output, dict):
        continue
    for keyWordGroup in output.get("keywordGroups", []):
        if not isinstance(keyWordGroup, dict):
            continue
        for keywordObj in keyWordGroup.get("keywords", []):
            if not isinstance(keywordObj, dict):
...
for researcher in researchers:
    for abs_obj in researcher.get("abstracts", []):
        if not isinstance(abs_obj, dict):
            continue
