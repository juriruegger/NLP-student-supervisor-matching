...
# Load CSV files
professor_data = pd.read_csv("embeddings_code/embeddings/embeddingsWithModernBERT.csv")

# Extract and count occurrences of organizational units
org_unit_counts = {}
...
# Embed text for BERT and SciBERT
...
# Embed text for ModernBERT (no padding, optimized for variable-length sequences)
def embed_text_modernbert(text, tokenizer, model):
...
student_embeddings = {
    f"text {idx + 1}": {
        ...
        "modernbert": embed_text_modernbert(text, modernbert_tokenizer, modernbert_model).detach()
    }
...
for _, row in professor_data.iterrows():
    ...
    embedding_modernbert = torch.tensor([float(x) for x in row["embedding_modernbert_768"].strip("[]").split(", ")], dtype=torch.float)
    ...
    for text_label, embeddings in student_embeddings.items():
        ...
        result[f"{text_label} modernbert score"] = cosine_similarity(embeddings["modernbert"], embedding_modernbert)
...
output_file = "embeddings_code/embeddings/test_resultsWithModernBERT.csv"
